...
int user_enter(int *fd,struct cli_users *user, sqlite3 *db)//登录
{
	look_account = 0;
	look_code = 0;
...
int user_register(int *fd,struct cli_users *user, sqlite3 *db)//注册
{
	char *errmsg = NULL;
	int ret;
	look_account = 0;
...
{   
	int i = 0;
	int save_salary = user->salary;
	char sava_department[128] = {0};
for(i=0;user->department[i];i++){
		sava_department[i] = user->department[i];
		}
	sqlite3_exec(db,"select * from stu;",callback,&user->account,NULL);
		if(look_account != 1){
			printf("帐号不存在\n");
		...
		user->salary = save_salary;
		for(i=0;sava_department[i];i++){
		user->department[i] = sava_department[i];
		}
	user->department[i] = sava_department[i];
		look_code = 0;
look_account = 0;
...
static int callback(void*para,int f_num,char ** f_value,char ** f_name)//回调函数
{                                                                 
	int i =-1,j=-1,k = 0;                             	
	for(i=0;i<f_num;i++){                                         
